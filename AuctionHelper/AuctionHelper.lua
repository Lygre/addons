...
windower.register_event('unhandled command', function(...)
	local commands = {...}
	local plzone = windower.ffxi.get_info()
	commands[1] = commands[1]:lower()
	...
	if actions or (not plzone.mog_house and not zones.dbox:contains(res_zones[windower.ffxi.get_info().zone].name)) then return end
	local now = os.clock()
